...
//			_data = new T;
	...
	size_t remove(const size_t& i)
	{
		if (i >= _size) return _size;
		if (i == _size - 1) return removeBack();

		for (size_t x = i; x < _size - 2; ++x)
			*(_data + x) = *(_data + x + 1);
		++_capacity;
		return this->size();
	}
	...
	// Remove the last element by pretending it isn't there
	size_t removeBack()
	{
		++_capacity;
		return this->size();
	...
	inline const T* operator->(void) const { return _data; }
